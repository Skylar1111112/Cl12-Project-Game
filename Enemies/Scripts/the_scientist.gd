...
@onready var return_button: CharacterBody2D = $"../Player/Game_Over/Button"
...
#reload
	if game_manager.freezeAll == false and player != null and dead == false:
		if reloading == false:
...
func _on_death_timer_timeout() -> void:
	...
	if win == false:
		game_over_animation.play("Win")
		return_button.return_button_visible = true
		win = true
		game_manager.freezeAll = true
...
func _on_whirlwind_attack_timer_timeout() -> void:
	if physical_attack == true and dead == false:
		game_manager.damage_player(2)


func _on_whirlwind_attack_area_body_exited(body: Node2D) -> void:
	if body == player:
		whirlwind_attack_timer.stop()
